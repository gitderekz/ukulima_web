...
export default function Dashboard() {
  ...
  const loadStats = async () => {
    const farmers = await db.farmers.findAll();
    ...
    const todayPurchases = purchases.filter((p) => p.purchaseDate.startsWith(today)).length;

    const totalRevenue = purchases.reduce((sum, p) => sum + p.totalAmount, 0);
    const pendingBales = bales.filter((b) => b.status === 'pending').length;
