...
// // // import { useTranslation } from 'react-i18next';
// // // import { useAuthStore } from '../store/authStore';
// // // import { cropsAPI, gradesAPI, rebalesAPI, balesAPI } from '../../services/api';
// // // import type { Crop, Grade, Bale } from '../types';
// // // import { Search, Plus, Trash2, Printer, X, Edit, Package } from 'lucide-react';
// // // import { toast } from 'sonner';
...
// // //   const [mode, setMode] = useState<RebaleMode>('manual');
// // //   const [baleSearch, setBaleSearch] = useState('');
// // //   const [baleResults, setBaleResults] = useState<Bale[]>([]);
// // //   const [crops, setCrops] = useState<Crop[]>([]);
// // //   const [grades, setGrades] = useState<Grade[]>([]);
...
// // //   const [selectedBales, setSelectedBales] = useState<Bale[]>([]);
// // //   const [accumulatedRebales, setAccumulatedRebales] = useState<AccumulatedRebale[]>([]);
// // //   const [editingRebaleId, setEditingRebaleId] = useState<string | null>(null);
...
// // //   const loadPriceMappings = async () => {
// // //     try {
// // //       const response = await fetch('/api/prices/current');
// // //       const data = await response.json();
// // //       if (data.success && data.data) {
// // //         const priceMapObj: Record<string, number> = {};
// // //         data.data.forEach((price: any) => {
// // //           const key = `${price.cropId}-${price.gradeId}`;
// // //           priceMapObj[key] = price.price;
// // //         });
// // //         setPriceMap(priceMapObj);
// // //       }
// // //     } catch (error) {
// // //       console.error('Failed to load price mappings:', error);
// // //     }
// // //   };

// // //   const getPriceForCropGrade = (cropId: string, gradeId: string): number => {
// // //     const key = `${cropId}-${gradeId}`;
...
// // //     try {
// // //       // Search for bales that are available (not rebaled yet)
// // //       const response = await balesAPI.getAvailable();
// // //       if (response.success && response.data) {
// // //         const filtered = response.data.filter((bale: Bale) =>
// // //           bale.baleTag.toLowerCase().includes(query.toLowerCase()) &&
// // //           bale.status !== 'rebaled'
// // //         );
// // //         setBaleResults(filtered);
// // //       }
// // //     } catch (error) {
// // //       toast.error('Failed to search bales');
// // //     }
// // //   };

// // //   const selectBale = async (bale: Bale) => {
// // //     if (selectedBales.find((b) => b.id === bale.id)) {
// // //       toast.error('Bale already added');
// // //       return;
// // //     }

// // //     if (selectedBales.length === 0) {
// // //       const price = getPriceForCropGrade(bale.cropId, bale.gradeId);
// // //       setRebaleForm({
// // //         ...rebaleForm,
// // //         cropId: bale.cropId,
// // //         gradeId: bale.gradeId,
// // //         price: price,
// // //         sourceBaleIds: [bale.id],
// // //       });
// // //       setSelectedBales([bale]);
// // //     } else {
// // //       if (bale.cropId !== rebaleForm.cropId || bale.gradeId !== rebaleForm.gradeId) {
...
// // //       setRebaleForm({
// // //         ...rebaleForm,
// // //         sourceBaleIds: [...(rebaleForm.sourceBaleIds || []), bale.id],
// // //       });
// // //       setSelectedBales([...selectedBales, bale]);
// // //     }
...
// // //     // Check if rebale tag is unique in current list
// // //     const existingTag = accumulatedRebales.find((r) => r.rebaleTag === rebaleForm.rebaleTag);
// // //     if (existingTag && !editingRebaleId) {
// // //       toast.error('Rebale tag already exists');
// // //       return;
...
// // //         accumulatedRebales.map((r) =>
// // //           r.id === editingRebaleId
// // //             ? { ...rebaleForm, id: r.id, mass: mass.toString(), totalAmount, sourceBaleIds: rebaleForm.sourceBaleIds }
// // //             : r
// // //         )
// // //       );
// // //       toast.success('Rebale updated');
...
// // //         mass: mass.toString(),
// // //         totalAmount,
// // //         sourceBaleIds: rebaleForm.sourceBaleIds,
// // //       };
...
// // //     setLoading(true);
// // //     try {
// // //       // Prepare batch rebale data
// // //       const rebalesData = accumulatedRebales.map((rebale) => ({
// // //         cropId: rebale.cropId,
// // //         gradeId: rebale.gradeId,
// // //         totalMass: parseFloat(rebale.mass),
// // //         price: rebale.price,
...
// // //       // Use batch endpoint
// // //       const response = await rebalesAPI.createBatch(rebalesData);
      
// // //       if (!response.success) {
// // //         toast.error(`Failed to create rebales: ${response.message || response.error}`);
// // //         setLoading(false);
// // //         return;
// // //       }
...
// // //       // Clear the accumulated rebales
// // //       setAccumulatedRebales([]);
// // //     } catch (error) {
// // //       console.error('Error creating rebales:', error);
// // //       toast.error('Failed to create rebales');
// // //     } finally {
// // //       setLoading(false);
// // //     }
...
// // //                 <p className="text-sm text-gray-500 mt-1">Receipt: {receiptData.receiptNumber}</p>
// // //               )}
// // //             </div>
// // //             <button
// // //               onClick={closeReceipt}
...
// // //                         {rebale.crop?.name} - {rebale.grade?.name} | {rebale.mass} kg @ TZS {rebale.price}/kg
// // //                       </p>
// // //                       {rebale.sourceBaleIds && rebale.sourceBaleIds.length > 0 && (
// // //                         <p className="text-xs text-gray-500 mt-1">
// // //                           Source bales: {rebale.sourceBaleIds.length} bales
// // //                         </p>
// // //                       )}
// // //                     </div>
// // //                     <p className="font-semibold text-gray-900 dark:text-white">TZS {rebale.totalAmount.toLocaleString()}</p>
...
// // //                 Total Mass: {selectedBales.reduce((sum, b) => sum + b.mass, 0)} kg
// // //               </p>
// // //             </div>
// // //           </div>
// // //         )}
...
// // //                     setRebaleForm({ ...rebaleForm, cropId });
// // //                     // Auto-fill price when both crop and grade are selected
// // //                     if (cropId && rebaleForm.gradeId) {
// // //                       const price = getPriceForCropGrade(cropId, rebaleForm.gradeId);
// // //                       if (price) setRebaleForm(prev => ({ ...prev, price }));
// // //                     }
// // //                   }}
// // //                   className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-green-500"
// // //                 >
...
// // //                     // Auto-fill price when both crop and grade are selected
// // //                     if (rebaleForm.cropId && gradeId) {
// // //                       const price = getPriceForCropGrade(rebaleForm.cropId, gradeId);
// // //                       if (price) setRebaleForm(prev => ({ ...prev, price }));
// // //                     }
// // //                   }}
// // //                   className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-green-500"
...
// // //             onClick={handlePrintReceipt}
// // //             disabled={loading}
// // //             className="w-full py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-lg font-semibold flex items-center justify-center gap-2 transition-colors disabled:opacity-50"
// // //           >
// // //             {loading ? (
...
// // import { useTranslation } from 'react-i18next';
// // import { useAuthStore } from '../store/authStore';
// // import { cropsAPI, gradesAPI, rebalesAPI, balesAPI, pricesAPI } from '../../services/api';
// // import type { Crop, Grade, Bale } from '../types';
// // import { Search, Plus, Trash2, Printer, X, Edit, Package } from 'lucide-react';
// // import { toast } from 'sonner';
...
// //   const getPriceForCropGrade = async (cropId: string, gradeId: string): Promise<number> => {
// //     try {
// //       const response = await pricesAPI.getCurrentPrice(cropId, gradeId);
// //       if (response.success && response.data) {
...
// //     try {
// //       // Search for bales that are available (purchased and not rebaled)
// //       const response = await balesAPI.getAvailable();
// //       if (response.success && response.data) {
// //         const filtered = response.data.filter((bale: Bale) =>
// //           bale.baleTag.toLowerCase().includes(query.toLowerCase()) &&
...
// //     if (selectedBales.length === 0) {
// //       const price = await getPriceForCropGrade(bale.cropId, bale.gradeId);
// //       setRebaleForm({
// //         ...rebaleForm,
// //         cropId: bale.cropId,
// //         gradeId: bale.gradeId,
...
// //     // Auto-fill price when both crop and grade are selected
// //     if (cropId && rebaleForm.gradeId) {
// //       const price = await getPriceForCropGrade(cropId, rebaleForm.gradeId);
// //       if (price) setRebaleForm(prev => ({ ...prev, price }));
// //     }
// //   };
...
// //     // Auto-fill price when both crop and grade are selected
// //     if (rebaleForm.cropId && gradeId) {
// //       const price = await getPriceForCropGrade(rebaleForm.cropId, gradeId);
// //       if (price) setRebaleForm(prev => ({ ...prev, price }));
// //     }
// //   };
...
// import { cropsAPI, gradesAPI, rebalesAPI, balesAPI, pricesAPI } from '../../services/api';
// import type { Crop, Grade, Bale } from '../types';
// import { Search, Plus, Trash2, Printer, X, Edit, Package, Loader2 } from 'lucide-react';
// import { toast } from 'sonner';
...
//   const [crops, setCrops] = useState<Crop[]>([]);
//   const [grades, setGrades] = useState<Grade[]>([]);
//   const [availableBaleIds, setAvailableBaleIds] = useState<Set<string>>(new Set());
...
//   const [receiptData, setReceiptData] = useState<any>(null);
//   const [loading, setLoading] = useState(false);
//   const [generatingRebale, setGeneratingRebale] = useState(false);
...
//       // Load available bales
//       await loadAvailableBales();
//     } catch (error) {
//       toast.error('Failed to load data');
//     } finally {
//       setLoading(false);
...
//   const loadAvailableBales = async () => {
//     try {
//       const response = await balesAPI.getAvailable();
//       if (response.success && response.data) {
//         const availableIds = new Set(response.data.map((bale: Bale) => bale.id));
//         setAvailableBaleIds(availableIds);
//       }
//     } catch (error) {
//       console.error('Failed to load available bales:', error);
//     }
//   };
...
//       const response = await balesAPI.getAvailable();
//       if (response.success && response.data) {
//         // Filter out already selected bales
//         const selectedIds = new Set(selectedBales.map(b => b.id));
//         const filtered = response.data.filter((bale: Bale) =>
//           bale.baleTag.toLowerCase().includes(query.toLowerCase()) &&
...
//     // Check if first bale being added
//     if (selectedBales.length === 0) {
//       const price = await getPriceForCropGrade(bale.cropId, bale.gradeId);
//       setRebaleForm({
//         ...rebaleForm,
...
//     setBaleResults(prev => prev.filter(b => b.id !== bale.id));
//     setBaleSearch('');
//     toast.success('Bale added');
//   };
...
//   const generateRebaleFromSelected = async () => {
//     if (!rebaleForm.rebaleTag) {
//       toast.error('Please enter a rebale tag');
//       return;
//     }
...
//     setGeneratingRebale(true);
    
//     try {
//       const totalMass = selectedBales.reduce((sum, b) => sum + b.mass, 0);
//       const totalAmount = totalMass * rebaleForm.price;

//       const newRebale: AccumulatedRebale = {
//         rebaleTag: rebaleForm.rebaleTag,
//         cropId: rebaleForm.cropId,
//         gradeId: rebaleForm.gradeId,
//         mass: totalMass.toString(),
//         price: rebaleForm.price,
//         sourceBaleIds: rebaleForm.sourceBaleIds,
//         id: `temp-${Date.now()}`,
//         totalAmount,
//       };
...
//       // Clear the form for next rebale
//       setRebaleForm({
//         rebaleTag: '',
//         cropId: '',
//         gradeId: '',
...
//       toast.success('Rebale generated successfully');
//     } catch (error) {
//       console.error('Error generating rebale:', error);
//       toast.error('Failed to generate rebale');
//     } finally {
//       setGeneratingRebale(false);
//     }
//   };

//   // const handleNextRebale = async () => {
//   //   if (mode === 'manual') {
//   //     // Manual mode: use the existing logic
//   //     if (!rebaleForm.rebaleTag) {
//   //       toast.error('Please enter rebale tag');
//   //       return;
//   //     }
...
//   //     const mass = parseFloat(rebaleForm.mass);
//   //     if (isNaN(mass) || mass <= 0) {
//   //       toast.error('Invalid mass value');
//   //       return;
//   //     }
...
//   //     });
//   //   } else {
//   //     // Track mode: use generateRebaleFromSelected
//   //     await generateRebaleFromSelected();
//   //   }
//   // };
//   const handleNextRebale = async () => {
//     if (mode === 'manual') {
...
//       });
//     } else {
//       // Track mode: generate rebale from selected bales
//       if (!rebaleForm.rebaleTag) {
//         toast.error('Please enter rebale tag');
//         return;
//       }
...
//       // Calculate mass from selected bales (not from rebaleForm.mass)
//       const totalMass = selectedBales.reduce((sum, b) => Number(sum) + (Number(b.mass) || 0), 0);
...
//       const totalAmount = Number(totalMass) * Number(rebaleForm.price);
...
//                   mass: totalMass.toString(), 
//                   totalAmount, 
//                   sourceBaleIds: selectedBales.map(b => b.id)
//                 }
//               : r
//           )
//         );
...
//     setLoading(true);
//     try {
//       // Prepare batch rebale data - sourceBaleIds will be used to create RebaleBale entries
//       const rebalesData = accumulatedRebales.map((rebale) => ({
//         cropId: rebale.cropId,
//         gradeId: rebale.gradeId,
//         totalMass: parseFloat(rebale.mass),
//         price: rebale.price,
//         warehouseId: user?.warehouseId || null,
//         sourceBaleIds: rebale.sourceBaleIds || [], // These will go to rebale_bales table
//       }));
...
//   const { totalMass, totalAmount } = calculateTotals();
//   const selectedTotalMass = selectedBales.reduce((sum, b) => sum + b.mass, 0);
...
//                 <div className="mt-3 pt-2 border-t border-blue-200 dark:border-blue-800">
//                   <p className="text-sm font-medium text-blue-900 dark:text-blue-300">
//                     Total Mass: {selectedTotalMass} kg
//                   </p>
//                   {rebaleForm.price > 0 && (
//                     <p className="text-sm font-medium text-blue-900 dark:text-blue-300 mt-1">
//                       Total Value: TZS {(selectedTotalMass * rebaleForm.price).toLocaleString()}
//                     </p>
//                   )}
//                 </div>
//               </div>
...
//         {mode === 'track' && rebaleForm.price > 0 && selectedBales.length > 0 && (
//           <div className="mb-4 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
//             <p className="text-sm text-green-700 dark:text-green-400 mb-1">Rebale Preview</p>
//             <p className="text-2xl font-bold text-green-900 dark:text-green-300">
//               TZS {(selectedTotalMass * rebaleForm.price).toLocaleString()}
//             </p>
//             <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
//               Total Mass: {selectedTotalMass} kg @ TZS {rebaleForm.price}/kg
//             </p>
//           </div>
//         )}
...
//         <button
//           onClick={handleNextRebale}
//           disabled={loading || generatingRebale || (mode === 'track' && selectedBales.length === 0)}
//           className="w-full py-3 bg-green-600 hover:bg-green-700 text-white rounded-lg font-semibold flex items-center justify-center gap-2 transition-colors disabled:opacity-50"
//         >
//           {(loading || generatingRebale) ? (
//             <Loader2 className="w-5 h-5 animate-spin" />
//           ) : (
//             <Plus className="w-5 h-5" />
//           )}
//           {mode === 'track' 
//             ? (generatingRebale ? 'Generating...' : 'Generate Rebale')
//             : (editingRebaleId ? 'Update Rebale' : 'Add Rebale')}
//           {/* |{mode === 'track' 
//             ? (editingRebaleId ? 'Update Rebale' : 'Generate Rebale')
//             : (editingRebaleId ? 'Update Rebale' : 'Add Rebale')}   */}
//         </button>
//       </div>
...
export default function Rebale() {
  ...
  // Calculate selected total mass safely
  const selectedTotalMass = selectedBales.reduce((sum, b) => {
    const mass = typeof b.mass === 'string' ? parseFloat(b.mass) : b.mass;
  ...
  const generateRebaleFromSelected = async () => {
    ...
    try {
      const totalMass = selectedTotalMass;
      const price = typeof rebaleForm.price === 'string' ? parseFloat(rebaleForm.price) : rebaleForm.price;
      const totalAmount = totalMass * price;

      if (isNaN(totalMass) || totalMass <= 0) {
        toast.error('Invalid total mass calculation');
        setGeneratingRebale(false);
        return;
      }
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-white dark:bg-gray-800 rounded-xl p-8 border border-gray-200 dark:border-gray-700">
          <div className="flex justify-between items-start mb-6">
            ...
            <button onClick={closeReceipt} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg">
              <X className="w-6 h-6" />
            </button>
          </div>
          ...
          <div className="space-y-6">
                ...
                {receiptData.rebales.map((rebale: any, index: number) => (
                  <div key={index} className="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                    <div>
                      <p className="font-medium text-gray-900 dark:text-white">{rebale.rebaleTag}</p>
                      <p className="text-sm text-gray-600 dark:text-gray-400">
                        {rebale.crop?.name} - {rebale.grade?.name} | {parseFloat(rebale.mass).toFixed(2)} kg @ TZS {rebale.price}/kg
                      </p>
                      {rebale.sourceBaleIds && rebale.sourceBaleIds.length > 0 && (
                        <p className="text-xs text-gray-500 mt-1">Source bales: {rebale.sourceBaleIds.length} bales</p>
                      )}
                    </div>
                    <p className="font-semibold text-gray-900 dark:text-white">TZS {rebale.totalAmount.toLocaleString()}</p>
                  </div>
            ...
            <div className="flex gap-4">
              <button onClick={() => window.print()} className="flex-1 py-3 bg-green-600 hover:bg-green-700 text-white rounded-lg font-semibold flex items-center justify-center gap-2">
                <Printer className="w-5 h-5" /> Print Receipt
              </button>
              <button onClick={closeReceipt} className="flex-1 py-3 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-900 dark:text-white rounded-lg font-semibold">
                Close
              </button>
            </div>
          </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
        ...
        <div className="flex gap-4">
          ...
          >
            <Package className="w-5 h-5 inline mr-2" /> Manual Entry
          </button>
          <button
            onClick={() => {
              setMode('track');
          ...
          >
            <Search className="w-5 h-5 inline mr-2" /> Track Source Bales
          </button>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
              ...
              {baleResults.length > 0 && (
                <div className="absolute z-10 w-full mt-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                  {baleResults.map((bale) => {
                    const baleCrop = crops.find((c) => c.id === bale.cropId);
                    const baleGrade = grades.find((g) => g.id === bale.gradeId);
                    const baleMass = typeof bale.mass === 'string' ? parseFloat(bale.mass) : bale.mass;
                    return (
                      <button
                        key={bale.id}
                        onClick={() => selectBale(bale)}
                      ...
                      >
                        <p className="font-medium text-gray-900 dark:text-white">{bale.baleTag}</p>
                        <p className="text-sm text-gray-600 dark:text-gray-400">
                          {baleCrop?.name} - {baleGrade?.name} | {baleMass.toFixed(2)} kg
                        </p>
                      </button>
                    );
                  })}
              ...
              <div className="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto">
                  {selectedBales.map((bale) => {
                    ...
                    return (
                      <div key={bale.id} className="flex justify-between items-center text-sm">
                        <span className="text-blue-900 dark:text-blue-300">{bale.baleTag} - {baleMass.toFixed(2)} kg</span>
                        <button onClick={() => removeBale(bale.id)} className="text-red-600 hover:text-red-700">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
                ...
                <div className="mt-3 pt-2 border-t border-blue-200 dark:border-blue-800">
                  <p className="text-sm font-medium text-blue-900 dark:text-blue-300">
                    Total Mass: {selectedTotalMass.toFixed(2)} kg
                  </p>
                  {rebaleForm.price > 0 && (
                    <p className="text-sm font-medium text-blue-900 dark:text-blue-300 mt-1">
                      Total Value: TZS {(selectedTotalMass * rebaleForm.price).toLocaleString()}
        ...
        {mode === 'track' && rebaleForm.price > 0 && selectedBales.length > 0 && (
          <div className="mb-4 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Total Mass: {selectedTotalMass.toFixed(2)} kg @ TZS {Number(rebaleForm.price).toFixed(2)}/kg
            </p>
          </div>
        )}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
          ...
          <div className="space-y-2 mb-6 max-h-96 overflow-y-auto">
            {accumulatedRebales.map((rebale) => {
              ...
              const grade = grades.find((g) => g.id === rebale.gradeId);
              return (
                <div key={rebale.id} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                  <div className="flex-1">
                    <p className="font-medium text-gray-900 dark:text-white">{rebale.rebaleTag}</p>
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      {crop?.name} - {grade?.name} | {parseFloat(rebale.mass).toFixed(2)} kg @ TZS {rebale.price}/kg
                    </p>
                    {rebale.sourceBaleIds && rebale.sourceBaleIds.length > 0 && (
                      <p className="text-xs text-gray-500 mt-1">Source bales: {rebale.sourceBaleIds.length} bales</p>
                    )}
                  ...
                  <div className="flex items-center gap-4">
                    <p className="font-semibold text-gray-900 dark:text-white">TZS {rebale.totalAmount.toLocaleString()}</p>
                    <div className="flex gap-2">
                      <button onClick={() => editRebale(rebale)} className="p-2 hover:bg-blue-100 dark:hover:bg-blue-900 rounded-lg">
                        <Edit className="w-4 h-4 text-blue-600" />
                      </button>
                      <button onClick={() => deleteRebale(rebale.id)} className="p-2 hover:bg-red-100 dark:hover:bg-red-900 rounded-lg">
                        <Trash2 className="w-4 h-4 text-red-600" />
                      </button>
                    </div>
                  </div>
          ...
          <div className="border-t border-gray-200 dark:border-gray-700 pt-4 mb-6">
            <div className="grid grid-cols-3 gap-4 mb-4">
              ...
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-400">Total Mass</p>
                <p className="text-xl font-bold text-gray-900 dark:text-white">{totalMass.toFixed(2)} kg</p>
              </div>
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-400">Total Amount</p>
                <p className="text-xl font-bold text-green-600 dark:text-green-400">TZS {totalAmount.toLocaleString()}</p>
