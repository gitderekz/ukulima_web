...
import type { Bale, Crop, Grade, Rebale as RebaleType } from '../types';
...
export default function Rebale() {
  ...
  const searchBale = async (query: string) => {
    ...
    const allBales = await db.bales.findAll();
    const availableBales = allBales.filter(
      (b) => b.status === 'purchased' && b.baleTag.toLowerCase().includes(query.toLowerCase())
    );
    setBaleResults(availableBales);
  };
  ...
  const selectBale = async (bale: Bale) => {
    ...
    if (selectedBales.length === 0) {
      const cropGradePrice = await db.cropGradePrices.findByCropAndGrade(bale.cropId, bale.gradeId);
      setRebaleForm({
        ...rebaleForm,
        cropId: bale.cropId,
        gradeId: bale.gradeId,
        price: cropGradePrice?.price || bale.price,
  ...
  const handlePrintReceipt = async () => {
    ...
    // Create rebale records
    const createdRebales: RebaleType[] = [];

    for (const rebale of accumulatedRebales) {
      const created = await db.rebales.create({
        rebaleTag: rebale.rebaleTag,
        sourceBaleIds: rebale.sourceBaleIds || [],
        cropId: rebale.cropId,
        gradeId: rebale.gradeId,
        ...
        buyerId: user?.id || '',
        rebaleDate: new Date().toISOString().split('T')[0],
        status: 'stored',
      });

      createdRebales.push(created);

      // Update source bales status if in track mode
      if (rebale.sourceBaleIds && rebale.sourceBaleIds.length > 0) {
        for (const baleId of rebale.sourceBaleIds) {
          await db.bales.update(baleId, { status: 'rebaled' });
        }
      }
    }
    ...
    await db.auditLogs.create({
      userId: user?.id || '',
      action: 'CREATE_REBALE_BATCH',
      entityType: 'rebale',
      entityId: createdRebales.map((r) => r.id).join(','),
      details: `Created ${createdRebales.length} rebales in batch`,
      ipAddress: '127.0.0.1',
    });
