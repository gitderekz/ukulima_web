...
export default function Prices() {
  ...
  const loadData = async () => {
    const [pricesData, cropsData, gradesData] = await Promise.all([db.cropGradePrices.findAll(), db.crops.findAll(), db.grades.findAll()]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingPrice) {
      await db.cropGradePrices.update(editingPrice.id, data);
      toast.success('Price updated');
    } else {
      await db.cropGradePrices.create(data);
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Delete this price?')) {
      await db.cropGradePrices.delete(id);
