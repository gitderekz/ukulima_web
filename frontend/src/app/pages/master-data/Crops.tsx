...
export default function Crops() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        toast.error(response.message || 'Failed to load crops');
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let response;
      if (editingCrop) {
        response = await cropsAPI.update(editingCrop.id, formData);
        if (response.success) {
        ...
        response = await cropsAPI.create(formData);
        if (response.success) {
    ...
    if (window.confirm('Delete this crop?')) {
      ...
      try {
        ...
        if (response.success) {
          ...
          toast.error(response.message || 'Failed to delete crop');
        }
      } catch (error) {
