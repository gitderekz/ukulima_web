...
export default function Warehouses() {
  ...
  const loadData = async () => {
    const [whData, locData] = await Promise.all([db.warehouses.findAll(), db.locations.findAll()]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingWarehouse) {
      await db.warehouses.update(editingWarehouse.id, data);
      toast.success('Warehouse updated');
    } else {
      await db.warehouses.create(data);
