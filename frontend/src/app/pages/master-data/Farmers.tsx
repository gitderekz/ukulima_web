...
// //   );
// // }
// // frontend/src/app/pages/Farmers.tsx
// import { useState, useEffect } from 'react';
// import { useTranslation } from 'react-i18next';
// import { farmersAPI, locationsAPI } from '../../services/api';
// import type { Farmer, Location } from '../../types';
...
//         email: farmer.email || '',
//         idNumber: farmer.idNumber || '',
//         cppId: farmer.cppId || '',           // 🔥 NEW
//         extensionId: farmer.extensionId || '', // 🔥 NEW
//       });
//     } else {
//       setEditingFarmer(null);
//       setFormData({
...
//   // 🔥 Filter locations by type for dropdowns
//   const cppLocations = locations.filter(l => l.type === 'cpp');
//   const extensionLocations = locations.filter(l => l.type === 'street');
//   const streetLocations = locations.filter(l => l.type === 'street');
...
//                     <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{farmer.phone}</td>
//                     <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{location?.name || '-'}</td>
//                     <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">TZS {farmer.totalDebt?.toLocaleString() || '0'}</td>
//                     <td className="px-4 py-3 text-right">
//                       <div className="flex justify-end gap-2">
//                         <button
//                           onClick={() => handleOpenModal(farmer)}
...
//           <div className="bg-white dark:bg-gray-800 rounded-xl p-6 w-full max-w-2xl my-8 border border-gray-200 dark:border-gray-700">
//             <div className="flex justify-between items-center mb-6">
//               <h2 className="text-2xl font-bold text-gray-900 dark:text-white">
//                 {editingFarmer ? 'Edit Farmer' : 'Add Farmer'}
//               </h2>
...
//                   <div>
//                     <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">CPP *</label>
//                     <select
//                       value={formData.cppId}
//                       onChange={(e) => setFormData({ ...formData, cppId: e.target.value })}
//                       required
//                       className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-green-500"
//                     >
//                       <option value="">Select CPP</option>
//                       {cppLocations.map((loc) => (
...
//                   </div>
//                   <div>
//                     <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Extension Officer</label>
//                     <select
...
//                   </div>
//                   <div>
//                     <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Location (Street) *</label>
//                     <select
//                       value={formData.locationId}
//                       onChange={(e) => setFormData({ ...formData, locationId: e.target.value })}
//                       required
...
//                   type="submit"
//                   disabled={loading}
//                   className="flex-1 py-3 bg-green-600 hover:bg-green-700 text-white rounded-lg font-semibold transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
//                 >
...
export default function Farmers() {
  ...
  const handleOpenModal = (farmer?: Farmer) => {
    if (farmer) {
      ...
      setFormData({
        ...
        idNumber: farmer.idNumber || '',
        cppId: farmer.cppId || '',
        extensionId: farmer.extensionId || '',
      });
    } else {
      setEditingFarmer(null);
      setFormData({
  ...
  // 🔥 Filter locations by type for dropdowns
  const cppLocations = locations.filter(l => l.type === 'cpp');
  const extensionLocations = locations.filter(l => l.type === 'extension');
