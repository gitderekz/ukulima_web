...
import { authAPI, locationsAPI, warehousesAPI } from '../../../services/api';
...
export default function Signup() {
  ...
  const loadLocationsAndWarehouses = async () => {
    try {
      const [locsRes, whsRes] = await Promise.all([
        locationsAPI.getAll(),
        warehousesAPI.getAll(),
      ]);
      
      if (locsRes.success && locsRes.data) setLocations(locsRes.data);
      if (whsRes.success && whsRes.data) setWarehouses(whsRes.data);
    } catch (error) {
      toast.error('Failed to load locations and warehouses');
    }
  };
