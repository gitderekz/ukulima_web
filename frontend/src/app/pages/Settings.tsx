...
export default function Settings() {
    ...
    try {
      ...
      try {
        ...
        if (response.success && response.data?.settings) {
          ...
          setFormData(prev => ({
            ...prev,
            deductionPercentage: serverSettings.deductionPercentage?.toString() || '30',
            primaryColor: serverSettings.primaryColor || '#22c55e',
            secondaryColor: serverSettings.secondaryColor || '#3b82f6',
            language: serverSettings.language || 'en',
            currency: serverSettings.currency || 'TZS',
          }));
          setLastUpdated(serverSettings.updatedAt || new Date().toISOString());
        }
      } catch (apiError) {
        console.log('Could not fetch server settings, using local');
      }
    } catch (error) {
  ...
  const resetToDefaults = async () => {
    if (window.confirm('Reset all settings to default values?')) {
      ...
      setLastUpdated(defaults.updatedAt);
      i18n.changeLanguage('en');
      toast.success('Settings reset to defaults');
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
              ...
              />
              <p className="text-xs text-gray-500 dark:text-gray-500 mt-2">
                Current: {formData.deductionPercentage}% | Default: 30%
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-4 pt-6 border-t border-gray-200 dark:border-gray-700">
            <button
              type="submit"
              disabled={loading}
              className="flex-1 py-3 bg-green-600 hover:bg-green-700 text-white rounded-lg font-semibold flex items-center justify-center gap-2 transition-colors disabled:opacity-50"
            >
              {loading ? (
